...
function EventSourcedAggregateRoot(existingConstructor, eventHandlers){
	...
	function EventSourcedAggregateRootConstructor(emitHandler){
		...
		this.emit = function emit(type, payload){
			if(sealed){
				throw new Error('The event-sourced aggregate object has been sealed and will no longer accept events. This usually indicates an attempt to emit new events outside a repository call\'s consistency boundary - a programming error.');
			}
